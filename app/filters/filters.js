    ...
    return function (timestamp, invalidateTime) {
        ...
        var obj = {
            ...
            time: $filter('getFormattedTime')(
                d,
                false,
                cfg.zigbeecfg.time_format, d
            ),
            invalidateTime: (di ?
            $filter('getFormattedDate')(di) + ' '
            + $filter('getFormattedTime')(di, false, cfg.zigbeecfg.time_format)
                : ''),
...
angApp.filter('getFormattedDate', function (cfg) {
    return function (d) {
        ...
        switch (cfg.zigbeecfg.date_format) {
            case 'dd-mm-yyyy':
                return day + '-' + mon + '-' + year;
            case 'yyyy-mm-dd':
                return year + '-' + mon + '-' + day;
...
angApp.filter('deviceName', function (cfg, deviceService) {
    return function (deviceId, node) {
        ...
        if (deviceId == cfg.controller.zbeeNodeId) {
...
angApp.filter('getRoutesCount', function ($filter) {
    return function (ZigbeeAPIData, nodeId) {
        ...
        var getFarNeighbours = function (nodeId, exludeNodeIds, hops) {
            ...
            if (hops > 2) // Zigbee allows only 4 routers, but we are interested in only 2, since network becomes unstable if more that 2 routers are used in communications
                return [];

            var nodesList = [];
            angular.forEach(ZigbeeAPIData.devices[nodeId].data.neighbours.value, function (nnodeId, index) {
                if (!(nnodeId in ZigbeeAPIData.devices))
 ...
 * Set zigbeePlus icon
 */
angApp.filter('zigbeePlusIcon', function () {
    return function (input) {
        //var icon = 'fa fa-minus';
        var icon = '&nbsp';
        if (input === true) {

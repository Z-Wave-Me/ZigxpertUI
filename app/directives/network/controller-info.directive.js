angApp.directive('zigbeeMoreOptions', function (dataService, _, $http) {
  return {
    restrict: 'E',
    transclude: true,
    template: `
      <a ng-href="https://z-wave.me/hardware-capabilities/?uuid={{zigbeeMoreOptionsUUID}}" target="_blank">{{_t('getMoreOptions')}} <i class="fas fa-external-link-alt"></i></a>
    ...
    `,
    link: function ($scope) {
      $scope.zigbeeMoreOptionsUUID = $scope.master['controller.data.uuid'].replace(/^0+/, '');
      const currentCrc = $scope.master['controller.data.firmware.caps.crc.value'];
      $scope.updatable = false;
      $http({
        method: 'get',
        url:  'https://service.z-wave.me/hardware/capabilities/?uuid=' + $scope.zigbeeMoreOptionsUUID
      }).then(
        function (data) {
          if (data) {

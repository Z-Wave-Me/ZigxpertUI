angApp.directive('zigbeeExpertCommand', function (dataService, _, $filter) {
  return {
    restrict: 'E',
    replace: true,
    transclude: true,
        ...
        <div class="panel-footer">
            <div class="btn-group">
              <button type="button" class="btn btn-primary" ng-disable="status === 'loading'" ng-click="store()">
                      <zigbee-execute-button status="status"></zigbee-execute-button> {{options.action}}
              </button>
            </div>
            <div class="btn-group">
          ...
          <zigbee-commands-clipboard set-fn="setDirectiveFn(theDirFn)" ng-show="dirty">{{localOptions.url}}</zigbee-commands-clipboard>
        </div>
      </div>`,
    scope: {
      data: '=',
    ...
    controller: ['$scope', '$cookies', function zigbeeExpertCommandController($scope, $cookies) {
      $scope.localOptions = {
        get commandStyle() {
      ...
      $scope.store = function () {
        if ($scope.status === 'ready') {
          ...
          dataService.runZigbeeCmd(request).then(function () {
  ...
  return {
    ...
    template: `
      <div class="cfg-control-content">
        ...
        <zigbee-input style="flex-wrap: wrap;" type="data.type" ng-model="data.value" index="0"></zigbee-input>
    ...
    link: function (scope, element, attrs) {
      ...
      const destroyInit = scope.$on('zigbeeInput', function (_, {data}) {
        store = data;
      })
      ...
      scope.store = function (request, buttonName) {
        if (scope.status === 'ready') {
          ...
          dataService.runZigbeeCmd(request)
  ...
  return {
    ...
    link: function (scope, element, attr, ctrl) {
      ...
      const off = scope.$on('zigbeeInput', function (event, {index, data}) {
        store[index] = data;
        scope.updateIndex();
      })
...
}).directive('zigbeeInput', function () {
  return {
    restrict: 'E',
    replace: true,
    template: `
      <div 
        class="input-group zigbee-input" 
        ...
        <zigbee-bit-mask ng-if="_type === 'bitmask'" ng-model="local.data" size="type.bitmask.size"></zigbee-bit-mask>
      </div>
`,
    scope: {
      type: '=',
    ...
    link: function (scope, element, attr) {
      ...
      const zigbeeInput = scope.$watch('local.data', function () {
        scope.$emit('zigbeeInput', {index: scope.index, data: converter(scope._type, scope.local.data)});
      });
      scope.$on('destroy', function () {
        zigbeeInput();
      })
...
}).directive('zigbeeBitMask', ['$timeout', function ($timeout) {
  return {
    restrict: 'E',
    replace: true,
    template: `
...
angApp.directive('zigbeeExecuteButton', function () {
  return {
    restrict: 'E',
    scope: {
      status: '='
...
angApp.directive('zigbeeCommandDataViewer', function () {
  return {
    restrict: 'E',
    replace: true,
    scope: {
      ...
      <table class="table table-striped table-condensed" ng-repeat="(key, value) in data">
          ...
          <tbody>
            <tr ng-repeat="v in value">
                <td style="width: 1rem"><zigbee-clipboard-mini text="v.cmd | configCommandTableData: options.style"></zigbee-clipboard-mini></td>
...
angApp.directive('zigbeeKeyboardControl', function ($document) {
  return {
    restrict: 'A',
    link: function (scope) {
      if (scope.devices) {

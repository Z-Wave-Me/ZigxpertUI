...
appController.controller('RSSIReportController', function ($scope, $filter, $timeout, $interval, dataService, cfg, $q, _) {
    const allSettled = function () {
        ...
        $q.allSettled(promises).then(function (response) {
            ...
            if (ZigbeeAPIData.state === 'rejected' || packetApi.state === 'rejected' || rssiChart.state === 'rejected') {
                alertify.alertError($scope._t('error_load_data'));
                return;
            }

            // Success - zigbeeData
            if (ZigbeeAPIData.state === 'fulfilled' && packetApi.state === 'fulfilled' && rssiChart.state === 'fulfilled') {
                const packetsPerDev = {}
                const report = []
                ...
                for (const node in packetsPerDev) {
                    ...
                    report.push({nodeId: +node, name: $filter('deviceName')(node, ZigbeeAPIData.value.devices[node]), RSSI, std})
                }

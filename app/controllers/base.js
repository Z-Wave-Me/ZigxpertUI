...
appController.controller('BaseController', function ($scope, $rootScope, $cookies, $filter, $location, $anchorScroll, $window, $route, $interval, $timeout, cfg, dataService, deviceService, myCache) {
   ...
   * @param {int} timeout
   */
  $scope.runZigbeeCmd = function (cmd, timeout, hideError) {
    timeout = timeout || 1000;
    $scope.toggleRowSpinner(cmd);
    dataService.runZigbeeCmd(cfg.store_url + cmd).then(function (response) {}, function (error) {
      if (!hideError) {
   ...
   * @returns {undefined}
   */
  $scope.loadZigbeeConfig = function (nocache) {
    // Set config
  };
  ...
  $scope.setDongle = function () {
    dataService.getApi('zigbee_list').then(function (response) {
  ...
  $scope.setTimeStamp = function () {
    dataService.getApi('time', null, true).then(function (response) {
      ...
      cfg.zigbeecfg.time_zone = response.data.data.localTimeZone;
  ...
  $scope.loadBoxApiData = function () {
    dataService.loadZigbeeApiData().then(function (ZigbeeAPIData) {
      ...
      //var homeId = ZigbeeAPIData.controller.data.homeId.value;
      var zbeeNodeId = ZigbeeAPIData.controller.data.nodeId.value;
      //var APIVersion = ZigbeeAPIData.controller.data.APIVersion.value;
      //var showAnalytics = true;
      // Changes MK
      //$scope.boxData.controller.controllerState = ZigbeeAPIData.controller.data.controllerState.value;
      // Rewrite config
      var cfgController = {
        //homeName: ZigbeeAPIData.controller.data.homeName.value || cfg.controller.homeName,
        //homeId: homeId,
        //homeIdHex: '0x' + ('00000000' + (homeId + (homeId < 0 ? 0x100000000 : 0)).toString(16)).slice(-8),
        hasDevices: hasDevices < 2 ? false : true,
        zbeeNodeId: zbeeNodeId,

...
appController.controller('UzbController', function ($scope, $timeout, $window, cfg, dataService, deviceService) {
     ...
     *
     */
    $scope.loadZigbeeData = function (token) {
        dataService.loadZigbeeApiData().then(function(ZigbeeAPIData) {
            $scope.token.load = token;
            var vendorId = parseInt(ZigbeeAPIData.controller.data.manufacturerId.value, 10);
            ...
            $scope.currentVersion = ZigbeeAPIData.controller.data.APIVersion.value.replace(/^0+/, '');
            $scope.bootloaderVersion = ZigbeeAPIData.controller.data.bootloader.crc.value;
            var tokenParam = (token ? '&token=' + token : '');
            var appVersion = ZigbeeAPIData.controller.data.APIVersion.value.split('.');
            var appVersionMajor = parseInt(appVersion[0], 10);
            var appVersionMinor = parseInt(appVersion[1], 10);
            var bootloaderCRC = ZigbeeAPIData.controller.data.bootloader.crc.value;
            var urlParams = '?vendorId=' + vendorId + '&appVersionMajor=' + appVersionMajor + '&appVersionMinor=' + appVersionMinor+ '&bootloaderCRC=' + bootloaderCRC + tokenParam + '&zbee=' + ZigbeeAPIData.controller.data.softwareRevisionVersion.value + '&uuid=' + ZigbeeAPIData.controller.data.uuid.value;
            //return;
            // Load uzb
            loadUzb(urlParams);
        });
    ...
    $scope.addToken = function (input) {
        ...
        $scope.loadZigbeeData(input.token);

...
appController.controller('ConfigHealthController', function ($scope, $routeParams, $timeout, $location, $cookies, $filter, $interval, cfg, deviceService, dataService) {
  ...
  $scope.load = function () {
    ...
    dataService.loadZigbeeApiData().then(function (ZigbeeAPIData) {
      ...
      $scope.health.ctrlNodeId = ZigbeeAPIData.controller.data.nodeId.value;
      var node = ZigbeeAPIData.devices[$routeParams.nodeId];
      /*if (!node || deviceService.notDevice(ZigbeeAPIData, node, $routeParams.nodeId)) {
          return;
      }*/
      ...
      setDevice(node);
      setData(ZigbeeAPIData, neighbours);
      $scope.refreshData();
  ...
  // Run Zigbee NOP Command
  $scope.runZigbeeNopCmd = function (cmd) {
    for (i = 0; i < 21; i++) {
   ...
   * @returns {undefined}
   */
  function setData(ZigbeeAPIData, neighbours) {

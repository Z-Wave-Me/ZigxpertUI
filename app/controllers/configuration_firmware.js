...
appController.controller('ConfigFirmwareController', function ($scope, $routeParams, $location, $cookies, $timeout, $filter, $interval, cfg, dataService, deviceService) {
    ...
    $scope.firmware = {
        ...
        interval: null,
        firmware_targets: [$scope._t('ota_zigbee_chip')],
        update: {
            show : false,
            status: '',
            progress: 0,
    ...
    $scope.load = function (nodeId) {
        dataService.loadZigbeeApiData().then(function (ZigbeeAPIData) {
            ...
            if(deviceService.hasCommandClass(node,122)){
                ...
                $scope.refreshZigbeeData($scope.deviceId);
            }else{
            ...
            var fromFrequency = ZigbeeAPIData.controller.data.frequency.value;
            var urlParams = 
    ...
    $scope.refreshZigbeeData = function (nodeId) {
        var refresh = function () {
            dataService.loadZigbeeApiData(true).then(function (ZigbeeAPIData) {
                var node = ZigbeeAPIData.devices[nodeId];

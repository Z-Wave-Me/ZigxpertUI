...
appController.controller('ZnifferController', function ($scope, $interval, $timeout, $cookies, $location, $http, $filter, cfg, dataService, deviceService, myCache, _) {
    ...
    $scope.loadZigbeeData = function() {
        dataService.loadZigbeeApiData(true).then(function(ZigbeeAPIData) {
            var appVersion = parseFloat(ZigbeeAPIData.controller.data.APIVersion.value);
            $scope.zniffer.promiscModeAvailable = appVersion >= 5.39; // Promisc appeared in this version
            $scope.zniffer.promiscModeEnabled = ZigbeeAPIData.controller.data.promiscMode.value;
        });
    };

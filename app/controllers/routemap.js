...
appController.controller('RouteMapController', function ($scope, $q,$interval, $filter, $window,$timeout,cfg, dataService,deviceService, myCache, _) {
    ...
    $scope.allSettled = function () {
        ...
        $q.allSettled(promises).then(function (response) {
            var ZigbeeAPIData = response[0];
            var packetApi = response[1];

            // zigbeeData and packet error message
            if (ZigbeeAPIData.state === 'rejected' || packetApi.state === 'rejected') {
                alertify.alertError($scope._t('error_load_data'));
                return;
            }

            // Success - zigbeeData
            if (ZigbeeAPIData.state === 'fulfilled' && packetApi.state === 'fulfilled') {
                //console.log(cfg.zigbeecfg.node_positions)
                // Is routemap image in the zigbee config?
                if(cfg.zigbeecfg.routemap_img){
                    $scope.routeMap.bcgImage = cfg.server_url + cfg.load_image + cfg.zigbeecfg.routemap_img;
                }/*else{
                    $scope.routeMap.bcgImage = cfg.server_url + cfg.load_image + 'floorplan.jpg';
                }*/
                // Set positions
                var positions = cfg.zigbeecfg.node_positions;
                var packets = packetApi.value.data.data;
                if (_.size(packets)) {
                    zrp = new ZigbeeRoutesPlotLib(new ZigbeeNetworkAnalyticsLib(ZigbeeAPIData.value, packets, positions));
    ...
    $scope.uploadFloorImage = function (files,info,spinner) {
        ...
        dataService.uploadApiFile(cmd, fd).then(function (response) {
            ...
            $scope.updateZigbeeConfig({routemap_img: files[0].name});
            $timeout(function () {
             alertify.dismissAll();
             $window.location.reload();
             }, 2000);
    ...
    $scope.saveNodesPositions = function(nodes){
        ...
        $scope.updateZigbeeConfig(input);
    }
     ...
     * @param {object} nodes
     */
    $scope.updateZigbeeConfig = function(input){
    ...
    $scope.manualRouteSet = function(route, deleted) {
        ...
        dataService.runZigbeeCmd(cfg.store_url + 'devices[' + src + '].AssignPriorityReturnRoute(' + dst + ', ' + route.join(',') + ')').then(function (response) {
            if (deleted) {

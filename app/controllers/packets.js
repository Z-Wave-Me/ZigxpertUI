appController.controller('PacketsController', function ($scope, dataService, $filter,) {
  ...
  (function () {
    Promise.all([dataService.getApi('packet_log', '', true), dataService.loadZigbeeApiData()])
      .then(([response, ZigbeeAPIData]) => {
        return [response.data.data, ZigbeeAPIData.devices, ZigbeeAPIData.controller.data.nodeId.value]

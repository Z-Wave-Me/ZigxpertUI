...
appController.controller('ConfigConfigurationController', function ($scope, $routeParams, $location, $cookies, $filter, $http, $timeout, $route, $interval,cfg, dataService, deviceService, myCache, _) {
    ...
    // Load data
    $scope.loadZigbeeData = function (nodeId) {
        dataService.loadZigbeeApiData().then(function(ZigbeeAPIData) {
            ...
            if(!$scope.hasConfigurationCc){
                $scope.refreshZigbeeData($routeParams.nodeId);
            }
     ...
     * Refresh zigbee data
     */
    $scope.refreshZigbeeData = function (nodeId) {
        var refresh = function() {
            dataService.loadJoinedZigbeeData(null).then(function(response) {
    ...
    $scope.updateFromDevice = function (cmd, hasBattery, deviceId, form,spin) {
        ...
        //$scope.refreshZigbeeData(deviceId);
    ...
    $scope.updateFromDeviceCfg = function (cmd, config, deviceId, form,spin) {
        ...
        angular.forEach(config, function (v, k) {
            if (v.confNum) {
                dataService.runZigbeeCmd(request + '(' + v.confNum + ')');
            }
        });
        //$scope.refreshZigbeeData(deviceId);
        $('#' + form + ' .cfg-control-content :input').prop('disabled', true);
    ...
    $scope.submitApplyConfigCfg = function (form, cmd, cfgValues, hasBattery, confNum, setDefault, hasData,spin) {
        ...
        switch (cmd['cluster']) {
                ...
                angular.forEach(xmlData, function (v, k) {
                    ...
                    if (confNum) {
                        if (confNum == v.confNum) {
                            dataService.runZigbeeCmd(cfg.store_url + configRequest);
                        }
                    } else {
                        dataService.runZigbeeCmd(cfg.store_url + configRequest);
                    }
            ...
            case '75':// Protection
                ...
                dataService.runZigbeeCmd(cfg.store_url + request);
                break;
            case '84':// Wakeup
                request += cmd.command + '(' + configValues.join(",") + ')';
                dataService.runZigbeeCmd(cfg.store_url + request);
        ...
        //$scope.refreshZigbeeData(cmd['id']);
        if (confNum) {
     ...
     * Set zigbee data
     */
    function setData(ZigbeeAPIData, nodeId) {
        var node = ZigbeeAPIData.devices[nodeId];
        if (!node) {
            return;
        }
        ...
        new Promise(function (resolve) {
            ...
            setCont(node, nodeId, config, ZigbeeAPIData);
        })
        ...
        //             myCache.put(zddXmlFile, zddXml);
        //             setCont(node, nodeId, zddXml, ZigbeeAPIData);
        //         },
        //             function (error) {
        //                 setCont(node, nodeId, null, ZigbeeAPIData);
        //             });
        //     } else {
        //         setCont(node, nodeId, cachedZddXml, ZigbeeAPIData);
        //     }
        // } else {
        //     setCont(node, nodeId, null, ZigbeeAPIData);
        // }
     ...
     * Set all conts
     */
    function setCont(node, nodeId, zddXml, ZigbeeAPIData) {
        if (!zddXml) {
            $scope.noZddx = true;
        ...
        dataService.getCfgXml().then(function (cfgXml) {
            ...
            $scope.wakeupCont = deviceService.configWakeupCont(node, nodeId, ZigbeeAPIData, cfgXml);
            $scope.protectionCont = deviceService.configProtectionCont(node, nodeId, ZigbeeAPIData, cfgXml);
            $scope.switchAllCont = deviceService.configSwitchAllCont(node, nodeId, ZigbeeAPIData, cfgXml);

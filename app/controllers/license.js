...
appController.controller('LicenseController', function($scope, $timeout, dataService) {
    ...
    $scope.loadZigbeeData = function() {
        dataService.loadZigbeeApiData().then(function(ZigbeeAPIData) {
            //$scope.controllerUuid = ZigbeeAPIData.controller.data.uuid.value;
            var appVersion = ZigbeeAPIData.controller.data.APIVersion.value.split('.');
            $scope.controllerIsZeroUuid = parseInt("0x" + ZigbeeAPIData.controller.data.uuid.value, 16) === 0;
            // Form data
            $scope.formData.uuid = ZigbeeAPIData.controller.data.uuid.value;

...
appService.service('deviceService', function ($filter, $log, $cookies, $window, $location, cfg, _) {
    ...
    this.setDongle = function (dongle) {
        angular.extend(cfg, {
            'update_url': '/Zigbee.' + dongle + '/Data/',
            'store_url': '/Zigbee.' + dongle + '/Run/',
            'restore_url': '/Zigbee.' + dongle + '/Restore',
            'queue_url': '/Zigbee.' + dongle + '/InspectQueue',
            'fw_update_url': '/Zigbee.' + dongle + '/FirmwareUpdate',
            'zme_bootloader_upgrade': '/Zigbee.' + dongle + '/ZMEBootloaderUpgrade',
            'zme_firmware_upgrade': '/Zigbee.' + dongle + '/ZMEFirmwareUpgrade',
            'upload_bootloader': '/Zigbee.' + dongle + '/ZMEBootloaderUpgrade',
            'upload_firmware': '/Zigbee.' + dongle + '/ZMEFirmwareUpgrade',
            'license_load_url': '/Zigbee.' + dongle + '/ZMELicense',
            'stat_url': '/Zigbee.' + dongle + '/CommunicationStatistics',
            'postfixget_url': '/Zigbee.' + dongle + '/PostfixGet',
            'postfixadd_url': '/Zigbee.' + dongle + '/PostfixAdd',
            'postfixremove_url': '/Zigbee.' + dongle + '/PostfixRemove',
            'checklinks': '/Zigbee.' + dongle + '/CheckAllLinks',
            'zniffer_url': '/Zigbee.' + dongle + '/Zniffer',
            'communication_history_url': '/Zigbee.' + dongle + '/CommunicationHistory',
            'rssi_chart': '/Zigbee.' + dongle + '/RSSIGet',
            'call_all_nif': '/Zigbee.' + dongle + '/CallForAllNIF',
            'test_node': '/Zigbee.' + dongle + '/TestNode/',
            'network_statistics': '/Zigbee.' + dongle + '/Run/',
            'post_report_api': '/Zigbee.' + dongle + '/sendZWayReport',
            'reorg_run_url': '/Zigbee.' + dongle + '/NetworkReorganization',
            'reorg_log_url': '/Zigbee.' + dongle + '/GetReorganizationLog',
            'zddx_create_url': '/Zigbee.' + dongle + '/CreateZDDX/',
            'get_network_statistics': '/Zigbee.' + dongle + '/GetStatisticsData',
            'clear_network_statistics': '/Zigbee.' + dongle + '/ClearStatisticsData',
     ...
     */
    this.notDevice = function (ZigbeeAPIData, node, nodeId) {
        return false;
    };
    ...
    */
     ...
     */
    this.configGetNav = function (ZigbeeAPIData) {
        return configGetNav(ZigbeeAPIData);
    };
     ...
     *  Get expert commands
     */
    this.configGetCommands = function (methods, ZigbeeAPIData) {
        return configGetCommands(methods, ZigbeeAPIData);
    };
     ...
     *  Get interview stage
     */
    this.configInterviewStage = function (ZigbeeAPIData, id, languages) {
        return configInterviewStage(ZigbeeAPIData, id, languages);
    };
     ...
     *  Switch all cont
     */
    this.configSwitchAllCont = function (node, nodeId, ZigbeeAPIData, cfgXml) {
        return configSwitchAllCont(node, nodeId, ZigbeeAPIData, cfgXml);
    };

    /**
     * Protection cont
     */
    this.configProtectionCont = function (node, nodeId, ZigbeeAPIData, cfgXml) {
        return configProtectionCont(node, nodeId, ZigbeeAPIData, cfgXml);
    };

    /**
     * Wakeup cont
     */
    this.configWakeupCont = function (node, nodeId, ZigbeeAPIData, cfgXml) {
        return configWakeupCont(node, nodeId, ZigbeeAPIData, cfgXml);
    };
     ...
     *  Get config navigation devices
     */
    function configGetNav(ZigbeeAPIData) {
        var devices = [];
     ...
     *  Get expert commands
     */
    function configGetCommands(methods, ZigbeeAPIData) {
        var methodsArr = [];
        angular.forEach(methods, function (params, method) {
            //str.split(',');
            var cmd = {};
            var defaultValues = method_defaultValues(ZigbeeAPIData, methods[method]);
            cmd['data'] = {
                'method': method,
     ...
     *  Get interview stage
     */
    function configInterviewStage(ZigbeeAPIData, id, languages) {
        var istages = [];
        istages.push((ZigbeeAPIData.devices[id].data.nodeInfoFrame.value && ZigbeeAPIData.devices[id].data.nodeInfoFrame.value.length) ? '+' : '-');
        istages.push('&nbsp;');
        istages.push((0x86 in ZigbeeAPIData.devices[id].endpoints[0].clusters) ? (ZigbeeAPIData.devices[id].endpoints[0].clusters[0x86].data.interviewDone.value ? '+' : (ZigbeeAPIData.devices[id].endpoints[0].clusters[0x86].data.interviewCounter.value > 0 ? '.' : '&oslash;')) : '+'); // Version
        istages.push((0x72 in ZigbeeAPIData.devices[id].endpoints[0].clusters) ? (ZigbeeAPIData.devices[id].endpoints[0].clusters[0x72].data.interviewDone.value ? '+' : (ZigbeeAPIData.devices[id].endpoints[0].clusters[0x72].data.interviewCounter.value > 0 ? '.' : '&oslash;')) : '+'); // ManufacturerSpecific
        istages.push((0x60 in ZigbeeAPIData.devices[id].endpoints[0].clusters) ? (ZigbeeAPIData.devices[id].endpoints[0].clusters[0x60].data.interviewDone.value ? '+' : (ZigbeeAPIData.devices[id].endpoints[0].clusters[0x60].data.interviewCounter.value > 0 ? '.' : '&oslash;')) : '+'); // MultiChannel
        var moreCCs = false;
        for (var i in ZigbeeAPIData.devices[id].endpoints) {
            istages.push('&nbsp;');
            var endpoint = ZigbeeAPIData.devices[id].endpoints[i];
    ...
    function configConfigCont(node, nodeId, zddXml, cfgXml, lang, languages) {
        ...
        if (!zddXml.ZigbeeDevice.hasOwnProperty("configParams")) {
            return null;
        }
        var config_cont = [];
        var params = zddXml.ZigbeeDevice.configParams['configParam'];
        ...
        angular.forEach(params, function (conf_html, i) {
            ...
            // get value from the Zigbee data
            var config_zigbee_value = null;
            ...
            if (angular.isDefined(node.endpoints[0].clusters[0x70])) {
                if (node.endpoints[0].clusters[0x70].data[conf_num] != null && node.endpoints[0].clusters[0x70].data[conf_num].val.value !== "") {
                    config_zigbee_value = node.endpoints[0].clusters[0x70].data[conf_num].val.value;
                    conf_default = config_zigbee_value;
            ...
            if (cfgFile[conf_num] !== undefined) {
                ...
                if (config_zigbee_value !== null) {
                    config_config_value = config_zigbee_value;
                } else {
            ...
            switch (conf_type) {
                ...
                case 'rangemapped':
                    ...
                    conf_method_descr = {
                        ...
                        configZigbeeValue: config_zigbee_value,
                        confNum: conf_num,
                        confSize: conf_size
                    };
                    break;
     ...
     * Switch all cont
     */
    function configSwitchAllCont(node, nodeId, ZigbeeAPIData, cfgXml) {
        ...
        if (0x27 in node.endpoints[0].clusters) {
            ...
            var updateTime = $filter('isTodayFromUnix')(uTime);
            var isUpdated = (uTime > iTime ? true : false);
            var gui_descr = getMethodSpec(ZigbeeAPIData, nodeId, 0, 0x27, 'Set');
     ...
     * Protection cont
     */
    function configProtectionCont(node, nodeId, ZigbeeAPIData, cfgXml) {
        ...
        if (0x75 in node.endpoints[0].clusters) {
            ...
            var updateTime = $filter('isTodayFromUnix')(uTime);
            var isUpdated = (uTime > iTime ? true : false);
            var gui_descr = getMethodSpec(ZigbeeAPIData, nodeId, 0, 0x75, 'Set');
            var conf_default_value = 0;
     ...
     * Wakeup cont
     */
    function configWakeupCont(node, nodeId, ZigbeeAPIData, cfgXml) {
        var wakeup_cont = false;
        if (0x84 in node.endpoints[0].clusters) {
            var cfgFile = getCfgXmlParam(cfgXml, nodeId, '0', '84', 'Set');
            var wakeup_zigbee_min = (node.endpoints[0].clusters[0x84].data.version.value == 1) ? 0 : node.endpoints[0].clusters[0x84].data.min.value;
            var wakeup_zigbee_max = (node.endpoints[0].clusters[0x84].data.version.value == 1) ? 0xFFFFFF : node.endpoints[0].clusters[0x84].data.max.value;
            var wakeup_zigbee_value = node.endpoints[0].clusters[0x84].data.interval.value;
            var wakeup_zigbee_default_value = (node.endpoints[0].clusters[0x84].data.version.value == 1) ? 86400 : node.endpoints[0].clusters[0x84].data['default'].value; // default is a special keyword in JavaScript
            var wakeup_zigbee_nodeId = node.endpoints[0].clusters[0x84].data.nodeId.value;
            var uTime = node.endpoints[0].clusters[0x84].data.updateTime;
            var iTime = node.endpoints[0].clusters[0x84].data.invalidateTime;
            var updateTime = $filter('isTodayFromUnix')(uTime);
            var isUpdated = (uTime > iTime ? true : false);
            if (wakeup_zigbee_min !== '' && wakeup_zigbee_max !== '') {
                var gui_descr = getMethodSpec(ZigbeeAPIData, nodeId, 0, 0x84, 'Set');
                gui_descr[0].type.range.min = parseInt(wakeup_zigbee_min, 10);
                gui_descr[0].type.range.max = parseInt(wakeup_zigbee_max, 10);
                ...
                if (angular.isArray(cfgFile) && cfgFile.length > 0) {
                    ...
                    if (wakeup_zigbee_value != "" && wakeup_zigbee_value != 0 && wakeup_zigbee_nodeId != "") {
                        // not defined in config: adopt devices values
                        wakeup_conf_value = parseInt(wakeup_zigbee_value, 10);
                    } else {
                        // values in device are missing. Use defaults
                        wakeup_conf_value = parseInt(wakeup_zigbee_default_value, 10);
                    }
                    ;
                }
                ;
                wakeup_cont = {
                    ...
                    updateTime: updateTime,
                    isUpdated: isUpdated,
                    defaultValue: wakeup_zigbee_default_value,
                    showDefaultValue: wakeup_zigbee_default_value,
    ...
    /**
     * Get Zigbee session @returns {unresolved}
     */
    function sessionApi() {
        return $http({
            method: "get",
